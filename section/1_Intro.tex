...
IMU should not be conflated with prompt-based approaches. Gao \textit{et al.}~\cite{gao2025can} show that a model can be prompted to behave as though a knowledge cutoff had occurred, but the knowledge remains in the weights and returns the moment the prompt is withdrawn. IMU instead modifies the parameters, so that forgetting persists across sessions and cannot be undone by re-prompting. Prompting makes a model act as if it does not know, whereas IMU makes it not know, which is the only reading under which a right to erasure is meaningful.
