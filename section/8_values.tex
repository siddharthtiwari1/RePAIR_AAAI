\begin{table*}[htbp]
\centering
\caption{Comparison of STAMP with SoTA baselines on Llama-3-8B on harmful knowledge removal and misinformation removal ($Acc_\textit{f}\!\downarrow$, $Acc_\textit{r}\!\uparrow$) and personal data erasure ($F\text{-}RL\!\downarrow$, $R\text{-}RL\!\uparrow$). Utility is TinyStories perplexity$\downarrow$ and RTE is in minutes. The Oracle, trained only on $\mathcal{D}_r^{\textit{full}}$, is the upper bound.}
