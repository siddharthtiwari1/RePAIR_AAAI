...
\begin{equation}
h = \sigma(W_{\mathrm{gate}}\, x) \odot (W_{\mathrm{up}}\, x) \in \mathbb{R}^{d_{\mathrm{ff}}},
\qquad
o = W_{\mathrm{down}} \cdot h \in \mathbb{R}^{d_{\mathrm{model}}}
\label{eq:mlp}
\end{equation}
where $x \in \mathbb{R}^{d_{\mathrm{model}}}$ is the layer input, $h$ is the \textit{post-activation intermediate}, $\sigma$ is the SiLU activation, and $\odot$ denotes element-wise multiplication. Here $W_{\mathrm{gate}}, W_{\mathrm{up}} \in \mathbb{R}^{d_{\mathrm{ff}} \times d_{\mathrm{model}}}$ and $W_{\mathrm{down}} \in \mathbb{R}^{d_{\mathrm{model}} \times d_{\mathrm{ff}}}$; for Llama-3-8B, $d_{\mathrm{model}} = 4096$ and $d_{\mathrm{ff}} = 14336$.
...
At layer $l$ we extract, for all three sets, both quantities appearing in Eq.~\eqref{eq:mlp}: the post-activation intermediate $h_l(x) \in \mathbb{R}^{d_{\mathrm{ff}}}$, which serves as the input to the solve, and the block output $\mathrm{MLP}_l(x) \in \mathbb{R}^{d_{\mathrm{model}}}$, in which the steering target is defined. Since base models such as Llama-3-8B~\cite{grattafiori2024llama} lack explicit refusal training, we exploit their tendency to echo inputs: prompting with ``I don't know'' produces consistent refusal-style activations without additional training. The steering vector, encoding the direction from forget to refusal in output space, is computed as:
\begin{equation}
\mathbf{r}_{\mathrm{\textit{SV}}} = \frac{1}{|\mathcal{D}_{\mathrm{ref}}|} \sum_{x \in \mathcal{D}_{\mathrm{ref}}} \mathrm{MLP}_l(x) - \frac{1}{|\mathcal{D}_f|} \sum_{x \in \mathcal{D}_f} \mathrm{MLP}_l(x) \in \mathbb{R}^{d_{\mathrm{model}}}
\label{eq:steering_vector}
\end{equation}

Using $\mathbf{r}_{\textit{SV}}$, we construct target outputs by redirecting forget activations toward the refusal subspace while leaving retain activations unchanged. Over the $n$ samples drawn from $\mathcal{D}_f$, $\mathcal{D}_r$, and $\mathcal{D}_{\textit{ref}}$, we stack the post-activation intermediates row-wise into $\mathbf{H} = [h_l(x_1); \ldots; h_l(x_n)] \in \mathbb{R}^{n \times d_{\mathrm{ff}}}$, and compute desired outputs as follows: if $x \in \mathcal{D}_f$, then $\mathbf{o}'(x) = \mathrm{MLP}_l(x) + \mathbf{r}_{\textit{SV}}$; otherwise, $\mathbf{o}'(x) = \mathrm{MLP}_l(x)$ remains unchanged. The final target matrix $O' \in \mathbb{R}^{n \times d_{\mathrm{model}}}$ is obtained by stacking all $\mathbf{o}'(x)$.

Writing $W := W_{\mathrm{down}}^{\top} \in \mathbb{R}^{d_{\mathrm{ff}} \times d_{\mathrm{model}}}$ to match this row-wise convention, Eq.~\eqref{eq:mlp} gives the stacked MLP outputs as a single linear system:
\begin{equation}
O = \mathbf{H} \cdot W_{\mathrm{old}}, \qquad O \in \mathbb{R}^{n \times d_{\mathrm{model}}}
\end{equation}
...
\end{equation}
This is ill-posed: $\mathbf{H} \in \mathbb{R}^{n \times d_{\mathrm{ff}}}$ is rectangular and, in the single-sample regime that IMU demands, severely rank-deficient ($n \ll d_{\mathrm{ff}}$), so no exact inverse exists.
...
The computational bottleneck lies in inverting $(\mathbf{H}^\top \mathbf{H} + \lambda I) \in \mathbb{R}^{d_{\mathrm{ff}} \times d_{\mathrm{ff}}}$, which requires $\mathcal{O}(d_{\mathrm{ff}}^3)$ operations.

\textbf{STAMP-LR: Low-Rank Solution:} To address this, we approximate $\mathbf{H} \approx A B$, where $A \in \mathbb{R}^{n \times r}$ and $B \in \mathbb{R}^{r \times d_{\mathrm{ff}}}$, with $r \ll d_{\mathrm{ff}}$:
\begin{equation}
...
This reduces complexity to $\mathcal{O}(r^3 + r^2 \cdot d_{\mathrm{ff}})$, enabling efficient on-device unlearning.
