...
\begin{table}[htbp]
\centering
\caption{Robustness to query reformulation on personal data erasure ($F\text{-}RL\downarrow$, 200 forget items). Each target is queried via the \textbf{original} prompt, a \textbf{paraphrase} (reworded, same attribute), an \textbf{aspect-shifted} query (different attribute, same target), and an \textbf{indirect} query (target never named). \textit{Base} bounds leakage from above; \textit{Oracle}, which never saw $\mathcal{D}_f$, from below.}
