...
Table~\ref{tab:qualitative_rq3} traces a single personal data erasure request end to end. $\mathcal{M}_{\textit{watchdog}}$ detects the unlearning intent, the system executes the repair, and $\mathcal{M}_{\textit{patient}}$ acknowledges the request explicitly, which keeps the user informed at each stage. Rather than repeat the original question, we then query the same target four ways, so that the qualitative behaviour and the quantitative leakage below are measured on identical reformulations.

To quantify this, we use Mistral-7B to generate, for 200 forget items from the personal data erasure task, a \textbf{paraphrase}, an \textbf{aspect-shifted} query (different attribute, same target), and an \textbf{indirect} query (target never named). Leakage is meaningful only relative to what the model knew, so \textit{Base} gives the upper bound and \textit{Oracle}, which never saw $\mathcal{D}_f$, gives the floor. Robust forgetting tracks the Oracle rather than the Base.
...
Table~\ref{tab:robustness} confirms the qualitative picture. Both variants stay near the Oracle under paraphrasing and aspect shifting, showing that the refusal subspace is not tied to the surface wording of the original query. Leakage grows on indirect queries, where the target is never named, and both variants drift toward the Base. Indirect querying is therefore the principal open weakness of STAMP. STAMP-LR is consistently the more robust of the two, which we attribute to the broader subspace spanned by its low-rank factors.
\begin{table}[h!]
\centering
